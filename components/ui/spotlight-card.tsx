...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const getInlineStyles = (): React.CSSProperties => {
    const s: any = {
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 0) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 60) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
      )`,
  ...
  const beforeAfterStyles = `
    ...
    [data-glow]::before{background-image:radial-gradient(calc(var(--spotlight-size)*0.75) calc(var(--spotlight-size)*0.75) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(var(--hue,0) calc(var(--saturation,80)*1%) calc(var(--lightness,50)*1%) / var(--border-spot-opacity,0.9)),transparent 100%);filter:brightness(2);}
    [data-glow]::after{background-image:radial-gradient(calc(var(--spotlight-size)*0.5) calc(var(--spotlight-size)*0.5) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(0 100% 100% / var(--border-light-opacity,0.8)),transparent 100%);}
  ...
  return (
    <>
      ...
      <div
        ...
        className={`${getSizeClasses()} ${!customSize ? "aspect-[3/4]" : ""} rounded-2xl relative shadow-[0_1rem_2rem_-1rem_rgba(0,0,0,0.8)] backdrop-blur-[2px] ${className}`}
      >
        <div data-glow />
        {children}
      </div>
